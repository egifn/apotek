...
class BuatBarcodeController extends Controller
{
    public function index()
    {
        $data_barang_barcode = DB::table('m_produk')
            ...
            ->select('m_produk.id','m_produk.kode_produk','m_produk.nama_produk','m_produk.kode_cabang',
                    'm_cabang.nama_cabang','m_produk.barcode')
            ->whereNotIn('m_produk.status', ['1'])
            ->whereNotIn('m_produk.tgl_kadaluarsa', ['0000-00-00'])
            ->get();

        return view ('master_data.barcode.index', compact('data_barang_barcode'));
    }
    ...
    public function generate($id)
    {
        ...
        return view('master_data.barcode.view',compact('qr_code','qr_produk'));   
    }
    ...
    public function pdf($id)
    {
        $qr_data = DB::table('m_produk')
                    ...
                    ->select('m_produk.id','m_produk.kode_produk','m_produk.nama_produk','m_produk.kode_cabang',
                            'm_cabang.nama_cabang','m_produk.barcode','m_produk_unit_varian.harga_jual')
                    ->where('m_produk.id', $id)
                    ->get();
