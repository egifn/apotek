...
class LapPenjualanPanelController extends Controller
{
    public function index()
    {
        return view ('laporan.penjualan_panel.index');
    }
    ...
    public function getDataPenjualan(Request $request)
    {
        ...
        $data_penjualan = DB::table('tr_penjualan_h')
                    ...
                    ->select('tr_penjualan_h.kode_penjualan','tr_penjualan_h.tgl_penjualan','tr_penjualan_h.kode_cabang','m_cabang.nama_cabang','tr_penjualan_h.jenis_penjualan','tr_penjualan_h.status_bayar',
        ...
        if(!isset($request->value)) {
            //$data_penjualan->where('tr_penjualan_h.tgl_penjualan', Carbon::now()->format('Y-m-d'));
            $data_penjualan->where('tr_penjualan_h.jenis_penjualan', 'Panel')
    ...
    public function view(Request $request)
    {
        ...
        if($tombol_excel == 'excel'){
            ...
            return view ('laporan.penjualan_panel.view', compact('data_penjualan_excel'));
            ...
            $pdf = PDF::loadview('laporan.penjualan_panel.pdf', compact('data_penjualan_pdf'))->setPaper('a4', 'landscape');
            return $pdf->stream();
